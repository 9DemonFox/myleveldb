...
TEST(Coding, Varint32_DemonFox) {
  std::vector<uint32_t> vec {1, 128};
  for(uint32_t num: vec){
    std::string s;
    PutVarint32(&s, num);
    std::cout << bitint(num) << "-->" << bitstr(s) << std::endl;
  }
}
